# 🎬 SOOP 지피티 채팅 모니터링 봇

SOOP(아프리카TV) 방송의 채팅을 실시간으로 모니터링하여 밈 패턴을 감지하고 통계를 수집하는 봇입니다.

## 📌 주요 기능

### 1. 방송 상태 자동 감지

- 아프리카TV API로 LIVE 상태 자동 확인
- 폴링 주기: 방송 중 60초, 방송 종료 직후 9분 이내 180초(리방 대비), 피크 타임(16~18시) 180초, 기본 600초
- 방송 시작/종료 시 자동 세션 관리

### 2. 밈 패턴 감지 (5종류)

| 밈           | 패턴                                  | 예시                   |
| ------------ | ------------------------------------- | ---------------------- |
| **지창**     | `지[ㅡ\s~-]*창`                       | 지창, 지~창, 지ㅡ창    |
| **세신**     | `세[ㅡ\s~-]*신`                       | 세신, 세~신            |
| **짜장면**   | `짜[ㅡ\s~-]*장[ㅡ\s~-]*면`            | 짜장면, 짜~장면        |
| **ㄷㅈㄹㄱ** | `ㄷ[ㅡ\s~-]*ㅈ[ㅡ\s~-]*ㄹ[ㅡ\s~-]*ㄱ` | ㄷㅈㄹㄱ               |
| **ㅆㄷㄴ**   | ㅆㄷㄴ / 쌋다나 / 쌌다나              | ㅆㄷㄴ, 쌋다나, 쌌다나 |

### 3. Wave(물타기) 감지

| 조건      | 값        |
| --------- | --------- |
| 지속 시간 | 20초간    |
| 메시지 수 | 20개 이상 |
| 간격 타임아웃 | 10초 (이 시간 동안 밈 미감지 시 streak 리셋) |
| 쿨다운    | 60초      |

### 4. Hot Moment(급증 구간)

**하나의 Wave = 하나의 Hot Moment.** Wave가 확정될 때(20초간 20개 이상) 그 시점을 Hot Moment로 기록합니다. 방송 종료 시 `data/hot_moments/YYYY-MM-DD.json`에 저장됩니다.

### 5. 데이터 저장

- 방송 종료 시 `data/hot_moments/YYYY-MM-DD.json`에 Hot Moments 및 세션별 밈 총 횟수 저장
- `/stats`의 `history`는 메모리 상의 방송 기록(최대 50건, Wave 횟수 기준)

---

## 🌐 API 엔드포인트

| 엔드포인트            | 설명                                                       |
| --------------------- | ---------------------------------------------------------- |
| `GET /`               | 봇 실행 상태 확인                                          |
| `GET /health`         | 헬스체크                                                   |
| `GET /stats`          | 현재 세션 통계 조회 (밈별 Wave/총 감지 횟수, Hot Moments, 방송 기록) |
| `GET /history`        | Hot Moments가 저장된 날짜 목록 조회 (`data/hot_moments/` 기준) |
| `GET /history/{date}` | 특정 날짜 Hot Moments 조회 (Wave 확정 시점 + 세션별·날짜별 밈 총 횟수) |

### `/stats` 응답 필드

- `status`: `"LIVE"` | `"WAITING"`
- `broadcast_title`, `started_at`: 방송 제목, 방송 시작 시각(방송 중일 때만 값 있음)
- 밈별: `ji_chang_wave_count`, `total_ji_chang_chat_count`, `sesin_*`, `jjajang_*`, `djrg_*`, `sdn_*`
- `last_detected_at`: 마지막 밈 감지 시각
- `hot_moments`: 현재 세션의 Hot Moment 목록 (Wave 확정 시점)
- `history`: 메모리 상 방송 기록 (`broadcast_title`, 밈별 Wave 횟수 `total_*_wave_count`)

### `/stats` 응답 예시

```json
{
  "status": "WAITING",
  "broadcast_title": "방송 준비 중",
  "started_at": null,
  "ji_chang_wave_count": 0,
  "total_ji_chang_chat_count": 0,
  "sesin_wave_count": 0,
  "total_sesin_chat_count": 0,
  "jjajang_wave_count": 0,
  "total_jjajang_chat_count": 0,
  "djrg_wave_count": 0,
  "total_djrg_chat_count": 0,
  "sdn_wave_count": 0,
  "total_sdn_chat_count": 0,
  "last_detected_at": null,
  "hot_moments": [],
  "history": [
    {
      "date": "2026-02-07",
      "broadcast_title": "방송 제목",
      "total_ji_chang_wave_count": 7,
      "total_sesin_wave_count": 0,
      "total_jjajang_wave_count": 0,
      "total_djrg_wave_count": 0,
      "total_sdn_wave_count": 0
    }
  ]
}
```

### `/history/{date}` 응답 구조

- `date`, `last_updated`: 조회 날짜, 파일 마지막 수정 시각
- `sessions`: 해당 날짜에 저장된 방송 세션 목록 (배열: 복수형)
  - 각 세션: `broadcast_title`, `saved_at`, `hot_moments`, `total_ji_chang_wave_count`, `total_sesin_wave_count`, `total_jjajang_wave_count`, `total_djrg_wave_count`, `total_sdn_wave_count`
- `total_*_wave_count`: 해당 날짜 전체 Wave 횟수 합계(모든 세션)

**Hot Moment = Wave 확정 시점**이므로, `hot_moments`에는 20초간 20개 이상 감지되어 Wave가 확정된 순간들이 담깁니다.

---

## 🚀 실행 방법

### 로컬 실행

```bash
python soop_chat_ji.py
```

서버는 `http://0.0.0.0:8000`에서 실행됩니다.

### Fly.io 배포

```bash
fly deploy
```

---

## 🛠️ VOD 링크 생성기

Hot Moments 시간을 기반으로 VOD 타임스탬프 링크를 생성합니다.

### 사용법

```bash
python vod_link_generator.py
```

### 설정 파일

- `data/timestamp.json`: 밈 감지 데이터 (VOD 링크 생성 시 참조)
- `data/vod_config.json`: VOD 세그먼트 설정

---

## 📦 프로젝트 구조

```
soop-chatting-crawling/
├── soop_chat_ji.py          # 메인 봇 (채팅 모니터링 + FastAPI)
├── vod_link_generator.py    # VOD 링크 생성기
├── data/
│   ├── hot_moments/         # 날짜별 Hot Moments JSON (YYYY-MM-DD.json)
│   ├── timestamp.json       # 밈 감지 데이터 (VOD 생성용)
│   └── vod_config.json      # VOD 설정
├── Dockerfile
├── fly.toml
└── README.md
```

---

## 📋 기술 스택

- **언어**: Python 3.11+
- **프레임워크**: FastAPI
- **비동기**: asyncio, websockets
- **호스팅**: Fly.io (무료 티어)

---

## 🔧 설정값 (코드 기준)

| 항목                     | 값                                           |
| ------------------------ | -------------------------------------------- |
| Wave 조건                | 20초간 20개 이상 + 60초 쿨다운               |
| Wave streak 간격 타임아웃 | 10초                                         |
| Hot Moment               | Wave 확정 시 1건 기록 (동일 조건)            |
| 폴링 주기 (방송 중)      | 60초                                         |
| 폴링 주기 (방송 종료 직후) | 180초 (9분 이내)                             |
| 폴링 주기 (피크 16~18시) | 180초                                        |
| 폴링 주기 (기본)         | 600초                                        |
| 방송 기록 최대 건수      | 50건                                         |

---

## 📝 라이선스

MIT License
